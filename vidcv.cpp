...
int nCount=0;
IplImage *py, *pu, *pv, *pu_big, *pv_big, *image,*rgbImage,*hsvImage,*finalImage,*chanImage,*chan3Image;
   ...
   if (pData)
   {
      ...
      if (buffer->length)
      {
		...
		if (pData->pstate->graymode==0)
		{
			...
			cvCvtColor(image,rgbImage,CV_YCrCb2RGB);	// convert to RGB (slow)

			cvCvtColor(rgbImage, hsvImage, CV_RGB2HSV);			

			//cvDilate(rgbImage, rgbImage, NULL, 18);

			CvScalar lower = cvScalar(1, 1, 1); // Hue sat val
			CvScalar upper = cvScalar(200, 240, 240); // Hue sat val

			//cvInRangeS(hsvImage, lower, upper, chanImage);			

			//cvDilate(chanImage, chanImage, NULL, 10);
			//cvConvertImage(chan3Image, chanImage);
			//cvAnd(finalImage, rgbImage, chan3Image);

			gray=cvarrToMat(rgbImage);
...
////////////////////////////////
	...
	detectAndDraw( gray, cascade, nestedCascade, 1, true);
/////////////////////////
// END OF FACE RECO
/////////////////////////
...
/////////////////////////////////
	...
	int w=state.width;
	int h=state.height;
	rgbImage = cvCreateImage(cvSize(w,h), IPL_DEPTH_8U, 3);
	hsvImage = cvCreateImage(cvSize(w,h), IPL_DEPTH_8U, 3);
	finalImage = cvCreateImage(cvSize(w,h), IPL_DEPTH_8U, 3);
	chanImage = cvCreateImage(cvSize(w,h), IPL_DEPTH_8U, 1);
	chan3Image = cvCreateImage(cvSize(w,h), IPL_DEPTH_8U, 3);
